...
def play():
    ...
    background_change_score = 20  # Điểm số cần đạt để thay đổi hình nền
    ...
    backgrounds = [pygame.transform.scale(bg, (1200, 700)) for bg in backgrounds]
    ...
    while True:
                ...
                if event.key == pygame.K_a:
                    for bird in birds:
                        if bird.rect.colliderect(tam_rect) and bird.visible:
                            ...
                            remaining_time += 1  # Cộng thêm giây vào thời gian còn lại
                    ...
                    for box_rect in boxes:
                        if tam_rect.colliderect(box_rect):
                            ...
                            if event == "heal":
                                hp_width = min(300, hp_width + 50)  # Hồi thêm 30 máu, giới hạn tối đa là 300
                            elif event == "explosion":
                                remaining_time = max(0, countdown_duration - (current_time - countdown_start_time) // 1000)
                                remaining_time += 3 # Cộng thêm 3 giây vào thời gian còn lại
                                countdown_duration = remaining_time  # Cập nhật lại countdown_duration
                                countdown_start_time = current_time
                                score += 5
                                for bird in birds:
        ...
        if current_bird_time - last_bird_time > time_between_birds and bird_count < max_birds:
